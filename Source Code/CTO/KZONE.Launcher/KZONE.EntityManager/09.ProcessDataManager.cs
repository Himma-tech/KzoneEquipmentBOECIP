    ...
    public class ProcessDataManager : IDataSource
    {
        ...
        public IList<string> ProcessDataValues(string key)
        {
            ...
            try
            {
                ...
                if (key.Contains("A2CVD_CDC"))
                {
                    timeKey = key.Split('_')[4].ToString().Trim();//因命名方式，将"3"改为"4"，例：12_13_3AES09_STR_20220503101452479，适用于21038~21043，2022.05.03 changed by MIXI
                }
                else if (key.Contains("F2ISP_3_Pre_CLN") || key.Contains("F2ISP_3_Post_CLN"))
                {
                    timeKey = key.Split('_')[6].ToString().Trim();
                }
                else
                {
                    timeKey = key.Split('_')[3].ToString().Trim();
                }
        ...
        /// <summary>
        /// Delete ProcessData Files 目前保留180天
        /// </summary>
        public void DeleteProcessDataFiles()
        {
            try
            {
                ...
                foreach (string directoryPath in directoryList)
                {
                    DateTime checkTime = Directory.GetCreationTime(directoryPath).AddDays(180);
