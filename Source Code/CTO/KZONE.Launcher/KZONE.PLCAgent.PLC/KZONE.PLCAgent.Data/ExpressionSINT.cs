...
namespace KZONE.PLCAgent.PLC
{
	...
	/// </summary>
	public static class ExpressionSINT
	{
		/// <summary>
		/// SINT decode method
		/// </summary>		
		...
		public static short Decode(int evtStartAddress10, int itemWOffset, int itemWPoints, int itemBOffset, int itemBPoints, short[] iodata)
		{
			...
			return (short)(value[0] >> itemBOffset & (ulong)(Math.Pow(2.0, (double)itemBPoints) - 1.0));
		}
		...
		public static void Encode(string sintString, int itemWOffset, int itemWPoints, int itemBOffset, int itemBPoints, short[] iodata)
		{
			byte[] buffer = new byte[8];
			int bufOffset = 0;
			...
			ulong[] value = new ulong[1];
			Buffer.BlockCopy(buffer, 0, value, 0, 8);
			ulong uvalue = (ulong)((long)Convert.ToInt16(sintString));
			ulong mask = (ulong)(Math.Pow(2.0, (double)itemBPoints) - 1.0);
			if (itemBPoints < 16)
			{
				if (0uL > uvalue || mask < uvalue)
				{
					throw new ArgumentOutOfRangeException("sintString", string.Format("NOT 0 <= {0} <= 2^{1}-1", uvalue, itemBPoints));
				}
			}
