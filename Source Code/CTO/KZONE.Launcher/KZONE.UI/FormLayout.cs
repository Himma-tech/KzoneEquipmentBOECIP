    ...
    public partial class FormLayout : FormBase
    {
        ...
        private void Unit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    unit = ObjectManager.UnitManager.GetUnit(_unitKey.Substring(0, 2), int.Parse(_unitKey.Substring(3, 2)));
                    ...
                    GetJobDatatodgvJobData(_unitKey.Substring(0, 2), _unitKey.Substring(3, 2));
                }
                else
                {
                    unit = null;
        ...
        private void DataSetting_EQP()
        {
            try
            {
                ...
                if (eqp != null)
                {
                    ...
                    if (eqp.File.CurrentRecipeNo == null)
                    {
                        if (txtCurrentRecipeNo.Text != string.Empty) txtCurrentRecipeNo.Text = string.Empty;
                    }
                    else
                    {
                    ...
                    {
                        if (txtCurrentRecipeID.Text != eqp.File.CurrentRecipeID.Trim()) txtCurrentRecipeID.Text = eqp.File.CurrentRecipeID.Trim();
                    }
                    //Product Glass Count
                    ...
                    if (txtStopcom.Text != eqp.File.StopBitCommand.ToString()) txtStopcom.Text = eqp.File.StopBitCommand.ToString();
                    if (eqp.File.StopBitCommand == true)
                    {
                    ...
                    if (txtCleanCommand.Text != eqp.File.ForceClearOutMode.ToString()) txtCleanCommand.Text = eqp.File.ForceClearOutMode.ToString();
                ...
                else
                {
                    ...
                    txtCurrentRecipeNo.Text = string.Empty;
                ...
                if (query.Count != 0)
                {
                    ...
                    foreach (Job jobData in query)
                    {
                        dgvJobData.Rows.Add("Detail",
                              ...
                              jobData.GlassType,
                              jobData.GlassJudge,
                              jobData.ProcessSkipFlag,
                              jobData.LastGlassFlag,
                              jobData.CIMModeCreate,
                              jobData.SamplingFlag,
                              jobData.Reserved,
                              jobData.InspectionJudgeResult,
                              jobData.InspectionReservationSignal,
                              jobData.ProcessReservationSignal,
                              jobData.TrackingDataHistory,
                              jobData.EquipmentSpecialFlag,
                              jobData.GlassID,
                              jobData.SorterGrade,
                              jobData.GlassGrade,
                              jobData.FromPortNo,
                              jobData.TargetPortNo,
                              jobData.TargetSlotNo,
                              jobData.TargetCassetteID,
                              jobData.Reserve,
                              jobData.PPID
        ...
        private void txtStopcom_TextChanged(object sender, EventArgs e)
        {
            eqp = ObjectManager.EquipmentManager.GetEQP("L3");

            if (eqp != null && bool.Parse(txtStopcom.Text) == true)
            {
                eqp.File.StopBitCommand = false;
                eqp.File.ReceiveStep = eReceiveStep.InlineMode;
