...
namespace KZONE.UI
{
    public partial class MDIForm : Form, IApplicationContextAware
    {
        ...
        public static FormRecipeValidationResult FrmRecipeValidationMessage;
        ...
        private void MDIForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                labels.Add(button6);
                labels.Add(button8);
                if (OPIAp.ServerName == "KWF23637R")//目前僅該綫需要上報material數據，其他綫不做記錄
                {
                    labels.Add(button9);
            ...
            if (comBox.Tag != null)
            {
                switch (comBox.Tag.ToString())
                {
                    ...
                    case "MaterialHistory":
                        FrmHistory = new FormHistory(new HisTableParam("SBCS_MATERIALHISTORY"));
                        FrmHistory.lblCaption.Text = "MATERIAL HISTORY";
                        break;
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            {
                if (FrmRecipeValidationMessage != null)
                {
                    if (eqp.File.RecipeValidationMessageUpdate)
                    {
                        FrmRecipeValidationMessage.UpData(eqp);
                    }
                }
                else
                {
                    FrmRecipeValidationMessage = new FormRecipeValidationResult(OPIAp);
                }
            }
        }
