...
namespace KZONE.UI
{
    public partial class FormBCSMessage : FormBase
    {
        ...
        public FormBCSMessage(OPIInfo opiAp)
        {
            ...
            this.lblCaption.Text = string.Format("[{0}] BCS Message ", OPIAp.ServerName);
            this.tmrRefresh.Enabled = true;             
        }

        public void ShowHistoryPageAndNewMessage()
        {
            this.tbcMessage.SelectedIndex = 0;//切換到History

            //GetBCMessage(); //disable by sy.wu 2016/06/23
        ...
        private void tmrShow_Tick(object sender, EventArgs e)
        {
            try
            {
                if (int.Parse(ObjectManager.ParameterManager.Parameters[" BCSMessageDisplayTime"].Value.ToString()) ==0) // FormMainMDI.G_OPIAp.BCSMessageDisplayTime == 0)
                {
                    tmrShow.Enabled = false;
                    return;
                }

                if (DisplayTimer.Elapsed.Seconds > int.Parse(ObjectManager.ParameterManager.Parameters[" BCSMessageDisplayTime"].Value.ToString()))
                {
        ...
        #region GetBCMessage() from SBCS_TERMINALMESSAGEHISTORY disable by sy.wu
        //private void GetBCMessage()
        //{
        //    try
        //    {
        //        var Q_BCMessage = (from msg in OPIAp.HisDBCtx.SBCS_TERMINALMESSAGEHISTORies
        //                           orderby msg.UPDATETIME descending
        //                           select new { msg.UPDATETIME, msg.TRANSACTIONID, msg.LINEID, msg.CAPTION, msg.TERMINALTEXT }).Take(OPIAp.QueryMaxCount).ToList();
