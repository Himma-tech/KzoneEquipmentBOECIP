...
namespace KZONE.UI
{
    public static class UniTools
    {
        ...
        public static bool ExportToExcel(string title, PagedGridView pdgv, bool exportAll, out string errMessage)
        {
            ...
            {
                SaveFileDialog saveDialog = new SaveFileDialog();
                saveDialog.Filter = "*.csv*|*.csv|*.xls*|*.xls";
        ...
        public static List<ItemEntity> CreateItemEntity(string data)
        {
            try
            {
                List<ItemEntity> _lstData = new List<ItemEntity>();

                string[] _aryItem = data.Split(',');

                foreach (string _items in _aryItem)
                {
                    string nodeName = string.Empty;
                    string[] _item = _items.Split(':');
                    //if (FormMainMDI.G_OPIAp.Dic_Node.ContainsKey(_item[1]))
                    //{
                    //     nodeName = FormMainMDI.G_OPIAp.Dic_Node[_item[1]].NodeName;
                    //}

                    //if (_item.Length >= 2)
                    //{
                    //    ItemEntity _entity = new ItemEntity(_item[0], string.Format("{0}_{1}",_item[1],nodeName));

                    //    _lstData.Add(_entity);
                    //}
                    //else
                    //{
                    //    ItemEntity _entity = new ItemEntity(_item[0], string.Format("{0}_{1}", _item[1], nodeName));
                    ...
                    else
                    {
                        ...
                        if (opiap.Dic_Node.ContainsKey(_item[1]))
                        {
                            nodeName = opiap.Dic_Node[_item[1]].NodeName;
                            ItemEntity _entity = new ItemEntity(_item[0], string.Format("{0}_{1}", _item[1], nodeName));
                            _lstData.Add(_entity);
                        }
        ...
        public static string GetOPIParameterByKey(string keyWord, string subKey, string defaultValue)
        {
            ...
            try
            {
                ...
                var _var = from d in _ctxBRM.SBRM_OPI_PARAMETER
                           where d.LINEID.Equals(opiap.ServerName)&& d.KEYWORD.Equals(keyWord) && d.SUBKEY.Equals(subKey)
                           select d;
                ...
                SBRM_OPI_PARAMETER _opiParam = _var.ToList()[0];

                _value = _opiParam.ITEMVALUE;
        ...
        public static string GetGlassID(int slotNo)
        {
            ...
            OPIInfo opiap = OPIInfo.CreateInstance();
               string _fabID = opiap.ServerName.Substring(0, 2);//(只取前面2碼)
            string _timeCode = string.Empty;
            string _timeYear = DateTime.Now.Year.ToString().Substring(DateTime.Now.Year.ToString().Length - 1, 1);
            string _timeMonth = string.Empty;
            string _timeDay = string.Empty;
            ...
            switch (DateTime.Now.Month.ToString())
            {
                ...
                case "12":
                    ...
                    break;
                default:
                    _timeMonth = DateTime.Now.Month.ToString();
                    break;
            }
            if (DateTime.Now.Date.Day < 10)
            {
                _timeDay = DateTime.Now.Date.Day.ToString();
            }
            else
            {
                if ((DateTime.Now.Date.Day) + 55 < 73)
                    _timeDay = Convert.ToChar(DateTime.Now.Date.Day + 55).ToString();
                else if ((DateTime.Now.Date.Day) + 55 > 72 && (DateTime.Now.Date.Day) + 55 < 79)
                    _timeDay = Convert.ToChar(DateTime.Now.Date.Day + 56).ToString();
                else if ((DateTime.Now.Date.Day) + 55 > 78)
                    _timeDay = Convert.ToChar(DateTime.Now.Date.Day + 57).ToString();
            }
            ...
            //else
            {
                ...
                _glassID = _fabID + _timeYear + _timeMonth + _timeDay + "@GlassID" + Convert.ToChar(_data2).ToString() + Convert.ToChar(_data1).ToString();
        ...
        public static string GetGlassIDSlot(int slotNo)
        {
            string _glassID = string.Empty;
            if (slotNo <= 99)
            {
                _glassID = slotNo.ToString().PadLeft(2, '0');
            }
            else
            {
                int _slotNo = slotNo - 99;
                int _data1 = 0; //第一码
                int _data2 = 0;//第二码
                int _quotient = _slotNo / 24; //商数决定第一码
                int _remainder = _slotNo % 24; //余数决定第二码

                if (_remainder == 0)
                {
                    if (_quotient - 1 > 9)
                    {
                        _quotient = 54 + _quotient;
                    }
                    else
                    {
                        _quotient = _quotient - 1;
                    ...
                    _data1 = _quotient;
                    if (_data1 >= 73) _data1 = _data1 + 1;  //跳过 I  
                    if (_data1 >= 79) _data1 = _data1 + 1;  //跳过 O
                    _data2 = 90;
                }
                else
                {
                    if (_quotient > 9)
                    {
                        _quotient = 55 + _quotient;
                    }
                    _data1 = _quotient;
                    _data2 = _remainder + 65 - 1;
                    if (_data1 >= 73) _data1 = _data1 + 1;  //跳过 I 
                    if (_data1 >= 79) _data1 = _data1 + 1;  //跳过 O
                    if (_data2 >= 73) _data2 = _data2 + 1;  //跳过 I 
                    if (_data2 >= 79) _data2 = _data2 + 1;  //跳过 O

                }
                if (_data1 <= 9)
                    _glassID = _data1.ToString() + Convert.ToChar(_data2).ToString();
                else
                    _glassID = Convert.ToChar(_data1).ToString() + Convert.ToChar(_data2).ToString();
            }
            return _glassID;
        }
        ...
        //        var c = from msg in ctxBRM.SBRM_RECIPEs
        ...
        //        SBCS_OPIHISTORY _opiHistory = new SBCS_OPIHISTORY();
        ...
        //        FormMainMDI.G_OPIAp.HisDBCtx.SBCS_OPIHISTORY.InsertOnSubmit(_opiHistory);
        //        FormMainMDI.G_OPIAp.HisDBCtx.SubmitChanges();
        //        #endregion
        ...
        /// <summary>
        /// 對DataGridView做Double記憶體緩衝，以避免資料顯示延遲
        /// </summary>
        /// <param name="dgv">參照的DataGridView控制項名稱</param>
        public static void DoubleBufferDataGridView(DataGridView dgv)
        {
        ...
        //                    case "Judge":
        //                        _desc1 = "Port Judge";
        //                        _desc2 = GetEnumDescription<ePortJudge>(_num);
        //                        break;
        //                    case "ENABLED":
        //                        _desc1 = "Port Enable";
        ...
        //                //EquipmentCIMModeChangeRequest _cimModeChange = (EquipmentCIMModeChangeRequest)Spec.CheckXMLFormat(MsgXml);
        ...
        //            case "ForceClearOutCommand":

        //                #region ForceClearOutCommand
        //                _cmdKey = "ForceClearOutCommand";

        //                ForceClearOutCommand _forceClearOutCommand = (ForceClearOutCommand)Spec.CheckXMLFormat(MsgXml);

        //                _cmdData = string.Format("{0} Set Status[{1}]",
        //                    _forceClearOutCommand.BODY.COMMAND, _forceClearOutCommand.BODY.STATUS);
        ...
        //                #region RobotOperationModeCommand //disable by sy.wu 2016/07/19
        ...
        //                #region RobotModeChangeRequest //disable by sy.wu 2016/07/19
        ...
        //                #region RobotCommandRequest //disable by sy.wu 2016/07/19
        ...
        //            case "IndexerRunModeChangeRequest":

        //                #region IndexerRunModeChangeRequest
        //                _cmdKey = "IndexerRunMode";

        //                IndexerRunModeChangeRequest _indexerRunMode = (IndexerRunModeChangeRequest)Spec.CheckXMLFormat(MsgXml);

        //                _cmdData = string.Format("Local No [{0}], Indexer Run Mode Change[{1}]",
        //                    _indexerRunMode.BODY.EQUIPMENTNO, GetEquipmentIndexerModeDesc(_indexerRunMode.BODY.INDEXERRUNMODE));
        ...
        //                #region VCRStatusChangeRequest //disable by sy.wu 2016/07/25
        //                //_cmdKey = "VCRStatusChange";

        //                //DCRStatusChangeRequest _vcrStatusChange = (DCRStatusChangeRequest)Spec.CheckXMLFormat(MsgXml);

        //                //_cmdData = string.Format("Local No [{0}], VCR No [{1}] Change to [{2}]",
        //                //    _vcrStatusChange.BODY.EQUIPMENTNO, _vcrStatusChange.BODY.DCRNO, _vcrStatusChange.BODY.DCRMODE);

        //                break;
        //                #endregion

        //            case "StopBitChangeRequest":  //MPLCInterlock

        //                #region StopBitChangeRequest
        //                _cmdKey = "StopBitChange";

        //                StopBitChangeRequest _mplcInterlockChange = (StopBitChangeRequest)Spec.CheckXMLFormat(MsgXml);

        //                _cmdData = string.Format("Local No [{0}], Stop Bit No [{1}] Change to [{2}]",
        //                    _mplcInterlockChange.BODY.EQUIPMENTNO, _mplcInterlockChange.BODY.STOPBITNO, _mplcInterlockChange.BODY.STOPBIT);
        ...
        //                #region InspectionIdleTimeRequest  disable by sy.wu 2016/06/23
        ...
        //                    _createInspFile.BODY.CASSETTESEQNO, _createInspFile.BODY.CASSETTESLOTNO);
        ...
        //                #region LocalModeDenseDataSend //disable by sy.wu 2016/06/28
        ...
        //                #region PPKOfflineDenseDataSend  //disable by sy.wu 2016/06/28
        //                //_cmdKey = "PPK DenseMapDownload";
        ...
        //                        string.Format("Cassette Seq No [{0}], Job Seq No [{1}], Glass ID [{2}]", _job.CASSETTESEQNO, _job.CASSETTESLOTNO, _job.GLASSID);
        //                }
        ...
        //        FormMainMDI.G_OPIAp.HisDBCtx.SBCS_OPIHISTORY.InsertOnSubmit(_opiHistory); //modify by sy.wu 2016/07/01
        //        FormMainMDI.G_OPIAp.HisDBCtx.SubmitChanges();
        ...
        //            case "RTCDummyRunModeCommandReply":
        //                #region
        //                _cmdKey = "RTCDummyRunModeCommand";

        //                RTCDummyRunModeCommandReply _rtcDummyRunModeCommandReply = (RTCDummyRunModeCommandReply)Spec.CheckXMLFormat(MsgXml);

        //                _cmdData = string.Format("RTC Dummy Run Mode Command Reply");
        //                #endregion
        //                break;
        ...
        //                //EquipmentCIMModeChangeReply _cimModeChange = (EquipmentCIMModeChangeReply)Spec.CheckXMLFormat(MsgXml);
        ...
        //            case "ForceClearOutCommandReply":

        //                #region ForceClearOutCommandReply
        //                _cmdKey = "ForceClearOutCommand";
        ...
        //                #region RobotOperationModeCommandReply //disable by sy.wu 2016/07/19
        //                //_cmdKey = "RobotOperationMode";
        ...
        //                #region RobotModeChangeReply //disable by sy.wu 2016/07/19
        //                _cmdKey = "RobotModeChange";
        ...
        //                #region RobotCommandReply //disable by sy.wu 2016/07/19
        //                _cmdKey = "RobotCommand";
        ...
        //            case "IndexerRunModeChangeReply":

        //                #region IndexerRunModeChangeReply
        //                _cmdKey = "IndexerRunMode";

        //                IndexerRunModeChangeReply _indexerRunMode = (IndexerRunModeChangeReply)Spec.CheckXMLFormat(MsgXml);

        //                _cmdData = string.Format("Local No [{0}] Reply",
        //                    _indexerRunMode.BODY.EQUIPMENTNO);
        ...
        //            case "DCRStatusChangeReply":

        //                #region VCRStatusChangeReply //disable by sy.wu 2016/07/25
        //                //_cmdKey = "VCRStatusChange";

        //                //DCRStatusChangeReply _vcrStatusChange = (DCRStatusChangeReply)Spec.CheckXMLFormat(MsgXml);
        ...
        //            case "StopBitChangeReply":

        //                #region StopBitChangeReply
        //                _cmdKey = "StopBitChange";

        //                StopBitChangeReply _mplcInterlockChange = (StopBitChangeReply)Spec.CheckXMLFormat(MsgXml);
        ...
        //                #region InspectionIdleTimeReply   disable by sy.wu 2016/06/23
        //                //_cmdKey = "InspectionIdleTime";
        ...
        //                #region PPKLocalModeDenseDataSendReply  //disable by sy.wu 2016/06/28
        //                //_cmdKey = "PPK DenseMapDownload";
        ...
        //                #region PPKOfflineDenseDataSendReply  //disable by sy.wu 2016/06/28
        //                //_cmdKey = "PPK DenseMapDownload";
        ...
        //        UniAuto.UniBCS.HKC.OpiSpec.Message _msg = Spec.CheckXMLFormat(MsgXml);
        ...
        private static string GetProportionalNameDesc(string RuleName)
        {
            ...
            {
                FetchGlassProportionalName _ftechName = opiap.CurLine.FetchGlassProportionalNames.Find(r => r.ProportionalNameNo.Equals(_num));
        ...
        public static string GetEquipmentIndexerModeDesc(string IndexerModeNo)
        {
            ...
            {
                OPIInfo opiap = OPIInfo.CreateInstance();
                LineIndexerMode _indexer = opiap.CurLine.LineIndexerModes.Find(r => r.IndexerModeNo.Equals(_num));
        ...
        {
            OPIInfo opiap = OPIInfo.CreateInstance();
            if (opiap.Dic_Node.ContainsKey(NodeNo) == false) return RunModeValue;
            ...
            List<LineRunMode> _lstRunMode = opiap.Dic_Node[NodeNo].LineRunModes;
        ...
        public static void Reload_RecipeSetting()
        {
            ...
            var _check = (from _d in _ctxBRM.SBRM_NODE
                          where _d.SERVERNAME == opiap.CurLine.ServerName
            ...
            {
                if (opiap.Dic_Node.ContainsKey(_data.NodeNo))
                {
                    opiap.Dic_Node[_data.NodeNo].RecipeRegisterCheck = (_data.RecipeCheck == "Y" ? true : false);
                    opiap.Dic_Node[_data.NodeNo].RecipeParameterCheck = (_data.ParameterCheck == "Y" ? true : false);
                }
            }
        }
        ...
        {
            OPIInfo opiap = OPIInfo.CreateInstance();
            var q = (from node in opiap.Dic_Node.Values
                     select new
                     {
        ...
        //public static string CheckBodyLineName(UniAuto.UniBCS.HKC.OpiSpec.Body body)
        //{
        ...
        #region addRecipeTableHistory disable by sy.wu 2016/06/23
        ...
        #region [縮放layout 16/06/30 Rex 補]
        /// <summary>
        /// 將layout的預設尺寸取出來
        /// </summary>
        /// <param name="pnlLayout">主要layout載入的panel名稱</param>
        /// <param name="_layoutFontSize">用來儲存主要layout的字體大小</param>
        /// <returns>回傳xml內所有的長度</returns>
        public static string[] GetLayoutSize(Control pnlLayout, out float _layoutFontSize)
        {
            try
            {
                string _sizeTag = string.Empty;
                foreach (Control o in pnlLayout.Controls)
                {
                    _sizeTag += o.Width + "|" + o.Height + "|" + o.Top + "|" + o.Left + "|";

                    foreach (Control s in o.Controls)
                    {
                        _sizeTag += s.Width + "|" + s.Height + "|" + s.Top + "|" + s.Left + "|";
                    }
                }
                _layoutFontSize = pnlLayout.Font.Size;

                return _sizeTag.Split('|');
            }
            catch (Exception ex)
            {
                NLogManager.Logger.LogErrorWrite("","UniTools", MethodBase.GetCurrentMethod().Name, ex);
        ...
        /// 改變layout尺寸用
        /// </summary>
        /// <param name="_layoutSize">要修改的值(百分比為主e.g. 1.25)</param>
        public static void ReLayoutSize(Control pnlLayout, string[] layoutOriginSize, float layoutOriginFontSize, Double _layoutSize)
        {
            try
            {
                int _layoutcount = 0;
                foreach (Control o in pnlLayout.Controls)
                {
                    o.Width = (int)(int.Parse(layoutOriginSize[_layoutcount]) * _layoutSize);
                    o.Height = (int)(int.Parse(layoutOriginSize[_layoutcount + 1]) * _layoutSize);
                    o.Top = (int)(int.Parse(layoutOriginSize[_layoutcount + 2]) * _layoutSize);
                    o.Left = (int)(int.Parse(layoutOriginSize[_layoutcount + 3]) * _layoutSize);
                    float size = (float)(layoutOriginFontSize * _layoutSize);
                    _layoutcount += 4;
                    o.Font = new System.Drawing.Font(o.Font.FontFamily, size);
                    foreach (Control s in o.Controls)
                    {
                        s.Width = (int)(int.Parse(layoutOriginSize[_layoutcount]) * _layoutSize);
                        s.Height = (int)(int.Parse(layoutOriginSize[_layoutcount + 1]) * _layoutSize);
                        s.Top = (int)(int.Parse(layoutOriginSize[_layoutcount + 2]) * _layoutSize);
                        s.Left = (int)(int.Parse(layoutOriginSize[_layoutcount + 3]) * _layoutSize);
                        s.Font = new System.Drawing.Font(s.Font.FontFamily, size);
                        _layoutcount += 4;
                    }
                }
            }
    ...
    public class WorkbookMgr
    {
        ...
        private Stream DataTableToExcelStream(DataTable sourceDt, string sheetName, string titleName)
        {
            ...
            for (int p = 0; p < sheetCount; p++)
            {
                ...
                subTitleFormat.BorderBottom =  (CellBorderType)NPOI.SS.UserModel.BorderStyle.THIN;
        ...
        private Stream DataTableToExcelStream(DataTable sourceDt, string sheetName, string titleName, string[] header)
        {
            ...
            for (int p = 0; p < sheetCount; p++)
            {
                ...
                subTitleFormat2.BorderBottom =  (CellBorderType)NPOI.SS.UserModel.BorderStyle.THIN;
    ...
    {
        public static DataTable GetDataTable(DBSETDataContext db, string TableName, System.Linq.IQueryable query)
        {
            DataTable dt = null;
        ...
        public static DataSet GetDataSet(DBSETDataContext db, string TableName, System.Linq.IQueryable query)
        {
        ...
        public static DataTable GetDataTable(DBSETDataContext db, string TableName, string Sql)
        {
            DataTable dt = null;
        ...
        public static DataSet GetDataSet(DBSETDataContext db, string TableName, string Sql)
        {
            if (string.IsNullOrEmpty(TableName)) TableName = "table";
