            ...
            try
            {
               ...
               // Lst_Equipment = new List<Node>();
                ...
                foreach (SBRM_NODE sbrm_node in rstNODE)
                {
                    ...
                    Dic_Node.Add(sbrm_node.ATTRIBUTE, _Node);
                ...
                foreach (SBRM_UNIT r in rstUnit)
                {
                    ...
                    _unit.UnitNo = (int)r.UNITNO; //t3 UnitNo為string,HKC改為int sy.wu 2016/06/23
                    _unit.UnitType = r.UNITTYPE;
                    _unit.UseRunMode = r.USERUNMODE.ToString().ToUpper();
                    _unit.SubUnit = r.SUBUNIT;

                    Dic_Unit.Add(r.ATTRIBUTE.PadRight(3, ' ') + r.UNITNO.ToString().PadLeft(2, '0'), _unit); //t3 UnitNo為string,HKC改為int sy.wu 2016/06/23
                }
                #endregion
                ...
                if (File.Exists(_path) && File.Exists(_descPatch))
                {
                    ...
                    foreach (XElement _e in _element)
                    {
                        ...
                        EquipmentService.LinkSignalType _type_By_EquipementService = new EquipmentService.LinkSignalType();
                        ...
                        _type.FlowType = _e.Attribute("FlowType").Value.Trim();
                        _type.PathPosition = _e.Attribute("PathPosition").Value.Trim();

                        _type_By_EquipementService.UpStreamLocalNo = _e.Attribute("UpStream").Value.Trim();
                        _type_By_EquipementService.DownStreamLocalNo = _e.Attribute("DownStream").Value.Trim();
                        _type_By_EquipementService.SeqNo = _e.Attribute("SeqNo").Value.Trim();
                        _type_By_EquipementService.LinkType = _e.Attribute("LinkType").Value.Trim();
                        _type_By_EquipementService.TimingChart = _e.Attribute("TimingChart").Value.Trim();
                        _type_By_EquipementService.FlowType = _e.Attribute("FlowType").Value.Trim();
                        _type_By_EquipementService.PathPosition = _e.Attribute("PathPosition").Value.Trim();

                        //添加Link设定档---By MiXi 20240116
                        Dictionary<string, Dictionary<string, List<string>>> dicLinkPortConfig = new Dictionary<string, Dictionary<string, List<string>>>();
                        foreach (XElement trxMatch in _e.Elements("TrxMatch"))
                        {
                            Dictionary<string, List<string>> dicLinkStreamInfos = new Dictionary<string, List<string>>();
                            foreach (XElement stream in trxMatch.Elements())
                            {
                                List<string> trxNameList = new List<string>();
                                foreach (XElement item in stream.Elements())
                                {
                                    trxNameList.Add(item.Attribute("name").Value.Trim());
                                }
                                dicLinkStreamInfos.Add(stream.Name.ToString(), trxNameList);
                            }
                            _type.LinkKey = trxMatch.Attribute("name").Value;
                            _type_By_EquipementService.LinkKey = trxMatch.Attribute("name").Value;
                            dicLinkPortConfig.Add(trxMatch.Attribute("name").Value, dicLinkStreamInfos);
                        }
                        _type.TrxMatch = dicLinkPortConfig;
                        _type_By_EquipementService.TrxMatch = dicLinkPortConfig;

                        if (GetLinkSignalDesc(_type.LinkType, _typeDoc, strFabType))
                        {
                            Lst_LinkSignal_Type.Add(_type);
                            EquipmentService.Lst_LinkSignal_Type.Add(_type_By_EquipementService);
                        }
                    }
                }
                #endregion
