...
namespace KZONE.PLCAgent.PLC
{
	...
	/// </summary>
	public static class ExpressionLONG
	{
		/// <summary>
		/// LONG decode method
		/// </summary>		
		...
		public static uint Decode(int evtStartAddress10, int itemWOffset, int itemWPoints, int itemBOffset, int itemBPoints, short[] iodata)
		{
			...
			return (uint)(value[0] >> itemBOffset & (ulong)(Math.Pow(2.0, (double)itemBPoints) - 1.0));
		}
		...
		public static void Encode(string longString, int itemWOffset, int itemWPoints, int itemBOffset, int itemBPoints, short[] iodata)
		{
			byte[] buffer = new byte[8];
			int bufOffset = 0;
			...
			ulong[] value = new ulong[1];
			Buffer.BlockCopy(buffer, 0, value, 0, 8);
			ulong uvalue = (ulong)Convert.ToUInt32(longString);
			ulong mask = (ulong)(Math.Pow(2.0, (double)itemBPoints) - 1.0);
			if (0uL > uvalue || mask < uvalue)
			{
				throw new ArgumentOutOfRangeException("longString", string.Format("NOT 0 <= {0} <= 2^{1}-1", uvalue, itemBPoints));
			}
