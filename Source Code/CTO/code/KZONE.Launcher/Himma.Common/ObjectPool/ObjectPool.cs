...
namespace Himma.Common.ObjectPool
{
    ...
    public partial class ObjectPool<T> : IObjectPool<T>
    {
        ...
        protected internal List<Object<T>> _allObjects = new List<Object<T>>();
        protected internal ConcurrentStack<Object<T>> _freeObjects = new ConcurrentStack<Object<T>>();
        ...
        private void CheckAvailable(int interval)
        {
            new Thread(() =>
            {
                ...
                while (UnavailableException != null)
                {
                    ...
                    {
                        TestTrace.WriteLine($"【{Policy.Name}】Next recovery time: {DateTime.Now.AddSeconds(interval)} ({ex})", ConsoleColor.DarkYellow);
                    }
                }
