        ...
        private static void WriteFile(Item item)
        {
            try
            {
                if (item._file.GetFilename() != string.Empty)
                {
                    ...
                    string bak_fname = string.Format("{0}{1}.bak", item._info.DataFilePath, item._file.GetFilename());
                    ...
                    #region 刪除原檔案-因BC電腦被斷電時, 會不明原因產生空檔案, 因此不再刪除原檔, 改命名為備份檔
                    {
                    ...
                    #region 2. 原檔案重命名為備份檔
                    {
                        #region 若備份檔已存在, 先刪除備份檔
                        {
                            if (File.Exists(bak_fname))
                                File.Delete(bak_fname);
                            while (File.Exists(bak_fname))
                                Thread.Sleep(1);
                        }
                        #endregion

                        if (File.Exists(fname))
                            File.Move(fname, bak_fname);
                        while (File.Exists(fname))
                            Thread.Sleep(1);
                    }
                    #endregion
