    ...
    public partial class FormHistory : FormBase
    {
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_frmParam.TableName == "SBCS_MATERIALHISTORY")
                {
                    grbDateTime.Tag = "UPDATETIME";
                }
                ...
                string resultCnt = string.Format(" WHERE {0} BETWEEN '{1}' AND '{2}'",
                    ...
                    string.Format("{0}:00:00", dtpStartDate.Value.ToString("yyyy-MM-dd HH")),
                    string.Format("{0}:59:59", dtpEndDate.Value.ToString("yyyy-MM-dd HH")));
                Equipment eq = EntityManager.ObjectManager.EquipmentManager.GetEQP("L3");
                string lineID = "";
                if (_frmParam.TableName == "SBRM_RECIPE")
                {
                    lineID = $" AND LINEID='{eq.Data.LINEID}'";
                }
                if (_frmParam.TableName == "SBCS_MATERIALHISTORY")
                {
                    lineID = $" AND NODEID='{eq.Data.NODEID}'";
                }
                sbSql.Append(result);
                sbSql.Append(lineID);
                sbSqlCnt.Append(resultCnt);
                sbSqlCnt.Append(lineID);
                ...
                if (this._frmParam.TableName == "SBCS_MATERIALHISTORY")
                {
                    dgvData.ReadOnly = false;
                    foreach (DataGridViewRow _row in dgvData.Rows)
                    {
                        _row.ReadOnly = true;
                        _row.DefaultCellStyle.BackColor = MaterialColorCode(_row.Cells["MATERIALSTATUS"].Value.ToString());
                    }
                }
        ...
        private Color MaterialColorCode(string value)
        {
            ...
            {
                int index = 0;
                string[] keyWord = { "UNUSED", "MOUNT", "UNMOUNT", "PREPARE", "IN_USE", "LOAD_REQUEST", "UNLOAD_REQUEST" };
                for (; index < keyWord.Length; index++)
                {
                    if (value.Trim() == keyWord[index].Trim())
                    {
                        break;
                    }
                }
                switch (index)
                {
                    ...
                    case 5:
                        return Color.FromArgb(200, 73, 98);
                    case 6:
                        return Color.FromArgb(145, 103, 123);
            ...
            try
            {
                ...
                if (_frmParam.TableName == "SBCS_MATERIALHISTORY")
                {
                    ...
                    if (dgvData.CurrentRow != null)
                    {
                        ...
                        FrmMaterialDataDetail _frm = new FrmMaterialDataDetail(dgvData.CurrentRow) { TopMost = true };
                        _frm.ShowDialog();
