...
namespace KZONE.UI
{
    public partial class ucRBMethod : UserControl
    {
        ...
        List<SRCS_METHOD_DEF> ObjEdit=new List<SRCS_METHOD_DEF>();//Method Def 的集合 
        ...
        public void InitialRBMethod(string MethedType)
        {
            try
            {
                ...
                ObjEdit = ((from _obj in opiap.DBBRMCtx.SRCS_METHOD_DEF where _obj.METHODTYPE.Equals(MethedType) orderby _obj.METHODNAME select _obj).Distinct().ToList());
                List<string> _objName = new List<string>();
                //第一筆為空，並將ObjEdit中的ObjectName取出來
                _objName.Add("");
                _objName.AddRange( (from _obj in ObjEdit  orderby _obj.METHODNAME select _obj.METHODNAME).Distinct().ToList());
        ...
        public void cboObjectName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (cboObjectName.SelectedItem.ToString() != string.Empty)
                {
                    ...
                    _methodDef.Add("");
                    var _method = (from _Method in ObjEdit orderby _Method.METHODNAME where _Method.METHODNAME.Equals(cboObjectName.SelectedItem.ToString()) select _Method).Distinct().ToList();
        ...
        public void cboMethodName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cboMethodName.SelectedItem.ToString() != string.Empty)
                {
                    ...
                    var getContent = (from _obj in ObjEdit where _obj.METHODNAME.Equals(ItemID) && _obj.METHODNAME.Equals(cboObjectName.SelectedItem.ToString()) select _obj.DESCRIPTION).ToList();
