...
namespace KZONE.EntityManager
{
    ...
    {
        public Dictionary<string, Job> _entities = new Dictionary<string, Job>();
        ...
        public Job GetJob(string jobID)
        {
            try
            {
                lock (_entities)
                {
                    ...
                    Job ret = _entities.Values.OrderByDescending(w => w.CreateTime).FirstOrDefault(j => j.GlassID_or_PanelID.Trim() == jobID.Trim()); //.Select(w => w.GlassChipMaskBlockID == jobID)
                    if (ret != null)
                    {
                        return ret;
                    }
        ...
        public void SaveJobHistory(Job job, string eventName, string equipmentId, string equipmentNo, string unitNo, string unitId, string portNo, string slotNo)
        {
            try
            {
                ...
                his.UNITNO = unitNo;
                his.Cassette_Sequence_No = job.Cassette_Sequence_No.ToString();
                his.Job_Sequence_No = job.Job_Sequence_No.ToString();
                his.GlassID = job.GlassID_or_PanelID;
                his.Product_ID = job.Product_ID;
                his.Operation_ID = job.Operation_ID;
                his.Lot_ID = job.Lot_ID;
                his.CST_Operation_Mode = job.CST_Operation_Mode;
                his.Substrate_Type = job.Substrate_Type;
                his.Product_Type = job.Product_Type;
                his.Job_Type = job.Job_Type;
                his.Dummy_Type = job.Dummy_Type;
                his.Skip_Flag = job.Skip_Flag;
                his.Process_Flag = job.Process_Flag;
                his.Process_Reason_Code = job.Process_Reason_Code;
                his.LOT_Code = job.LOT_Code.ToString();
                his.Glass_Thickness = job.Glass_Thickness;
                his.Glass_Degree = job.Glass_Degree;
                his.Inspection_Flag = job.Inspection_Flag;
                his.Job_Judge = job.Job_Judge;
                his.Job_Grade = job.Job_Grade;
                his.Job_Recovery_Flag = job.Job_Recovery_Flag;
                his.Mode = job.Mode;
                his.Step_ID = job.Step_ID;
                his.VCR_Read_ID = "";//job.VCR_Read_ID;
                his.Master_Recipe_ID = job.Master_Recipe_ID;
                Line line = ObjectManager.LineManager.GetLine(equipmentId);
                if (line.Data.FABTYPE == "CELL")//equipmentId == "KWF23633L"
                {
                    his.PPID = job.PPID03;
                }
                else
