...
namespace KZONE.EntityManager
{
    public class RecipeManager
    {
        ...
        public void Init()
        {
            ...
            _recipeEntitys = ReloadRecipeByNo();
        }
        ...
        public RecipeEntityData FindOne(string nodeNo, string recipeNo)
        {
            try
            {
                if (this.HibernateAdapter != null)
                {
                    IList list2 = HibernateAdapter.GetObject_AND(typeof(RecipeEntityData),
                        new string[] {"NODENO", "RECIPENO"},
                        new object[] { nodeNo, recipeNo }, null, null);
        ...
        public void DeleteRecipeObject(string recipeNo, string nodeNo)
        {
            try

            {
                RecipeEntityData recipe = FindOne(nodeNo, recipeNo);
                if (recipe !=null)
                {
        ...
        public void UpateRecipeObject(RecipeEntityData upateRecipe)
        {
            try
            {
                RecipeEntityData recipe = FindOne(upateRecipe.NODENO, upateRecipe.RECIPENO);
                if (recipe != null)
                {
        ...
        public Dictionary<string, Dictionary<string, RecipeEntityData>> ReloadRecipe()
        {
            try
            {
                ...
                string hql = string.Format("from RecipeEntityData where LINEID = '{0}' order by CREATETIME desc", Workbench.ServerName);
                ...
                if (lists != null)
                {
                    foreach (RecipeEntityData entity in lists)
                    {
                        ...
                        if (!recipeEntitys[entity.LINEID].ContainsKey(entity.RECIPEID))
                        {
                            recipeEntitys[entity.LINEID].Add(entity.RECIPEID, entity);
                        }
                    }
                }
            ...
            try
            {
                ...
                if (lists != null)
                {
                    foreach (RecipeEntityData entity in lists)
                    {
                        ...
                        if (!recipeEntitys[entity.LINEID].ContainsKey(entity.RECIPENO))
                        {
